...
// env.EMAIL = sh(returnStdout: true, script: "python3 -c \"import wcommon as wc; wc.sendmail('${env.mailRecipients}', 'try', 'data'); exit()\"").trim() 
            ...
            env.validated = sh(returnStdout: true, script: "python3 -c \"import json,requests; print(json.dumps(json.loads(requests.get('http://10.88.48.21:5000/validated?branch=${env.branch}',verify=False)), indent=4));\"").replaceAll("\n","<BR>").trim()

...
class mHANDLE():
	...
	def PutRun(self, myID, payload):
		...
		if current == {}:
			data = json.loads(wc.REST_POST(self.url + '/vAgent?reportId=' + str(myID), verify=False, args=payload, convert_args=True))
		else:
			data = json.loads(wc.REST_PUT(self.url + '/vAgent?reportId=' + str(myID)))
		return(data)

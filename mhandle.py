...
class mHANDLE():
	...
	def UpdateRun(self, myID, preamble, data, ForceUpdate=False):
		...
		if myID not in self.payload.keys():
			# NEW POST
			self.payload[myID] = {'stdout_lines': [str(time.ctime(time.time())), str(potential['Response'].keys()), str(potential['response.request.body'].keys()), 'ForceUpdate:' + str(ForceUpdate), str(preamble) + str(data)]}
